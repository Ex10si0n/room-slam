...
class GridEncoderCNN(nn.Module):
    ...
    def forward(self, grid_maps: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            grid_maps: [B, 1, H, W]
        Returns:
            grid_context: [B, d_model]  (global context)
        """
...
class ColliderDecoder(nn.Module):
    ...
    def forward(
        ...
        if grid_context is not None:
            combined_context = combined_context + grid_context.unsqueeze(1)  # [B,1,D]
...
class TraceToColliderTransformer(nn.Module):
    ...
    def forward(
        ...
        if grid_maps is not None:
            grid_context = self.grid_encoder(grid_maps)  # [B, d_model]
        else:
            grid_context = None
